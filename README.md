...
- **Let-binding синтаксис** — `let+`, `let*`, `and+`, `and*` для удобной композиции
- **Богатый набор комбинаторов** — ready-to-use парсеры для частых задач

## Модули библиотеки

### Parser
Основной модуль с базовыми парсерами и комбинаторами.

### Syntax
Модуль с инфиксными операторами и let-binding синтаксисом для монадических и аппликативных операций.

### Expr_parser
Парсер арифметических выражений с обязательными скобками вокруг бинарных операций.
Грамматика: `expr ::= const | (expr op expr) | -expr`

### Infix_expr_parser  
Парсер арифметических выражений с приоритетами операторов (без обязательных скобок).
Поддерживает: `+`, `-`, `*`, `/`, унарный `-`, скобки.
Пример: `1 + 2 * 3` → `7`

### Json_parser
Простейший JSON парсер. Поддерживает: null, bool, number, string, array, object.
