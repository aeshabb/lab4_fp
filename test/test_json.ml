...
let json_testable = testable (fun fmt j -> Format.fprintf fmt "%s" (to_string j)) ( = )
...
let test_array_with_numbers () =
  ...
  check json_option "parse array with numbers"
    (Some (Array [ Number 1.0; Number 2.0; Number 3.0 ]))
    result
...
let test_simple_object () =
  ...
  check json_option "parse simple object"
    (Some (Object [ ("name", String "test"); ("value", Number 42.0) ]))
    result

let test_nested_object () =
  let result = parse "{\"user\": {\"name\": \"Alice\", \"age\": 30}}" in
  let expected =
    Some (Object [ ("user", Object [ ("name", String "Alice"); ("age", Number 30.0) ]) ])
  in
  check json_option "parse nested object" expected result

let test_array_of_objects () =
  let result = parse "[{\"x\": 1}, {\"y\": 2}]" in
  let expected = Some (Array [ Object [ ("x", Number 1.0) ]; Object [ ("y", Number 2.0) ] ]) in
...
let () = run "JSON Parser" [ ("JSON Parser", json_tests) ]
