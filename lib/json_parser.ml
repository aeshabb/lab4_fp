...
let json_value () = match !json_value_ref with Some p -> p | None -> failwith "json_value not initialized"
...
let json : json t =
  lazy_p (fun () ->
    skip_spaces *>
    (null_parser <|> bool_parser <|> number_parser <|> string_parser <|> array_parser <|> object_parser)
    <* skip_spaces)
