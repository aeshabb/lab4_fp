...
(** Парсер числа *)
let number : int t = skip_spaces *> integer <* skip_spaces
...
let term : expr t =
  ...
  and+ rest =
    many
      (let+ o = mul_op and+ operand = factor in
       (o, operand))
  in
...
let expression : expr t =
  ...
  and+ rest =
    many
      (let+ o = add_op and+ operand = term in
       (o, operand))
  in
  make_binop_left init rest
...
let parse_and_eval s = match parse s with Some e -> Some (eval e) | None -> None
